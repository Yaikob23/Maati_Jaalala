...
const UPLOAD_PRESET = "jabaa1";
const GALLERY_TAG = "family_memories1";
...
function initTheme() {
    ...
    if (themeToggleBtn) {
        themeToggleBtn.addEventListener('click', () => {
            ...
            localStorage.setItem('jabaa_theme', newTheme);
    ...
    files.forEach(file => {
        ...
        .then(data => {
            if (data.secure_url) {
                ...
                alert("Upload error: Ensure preset is set to Unsigned in Cloudinary.");
            }
        })
    ...
    fetch(`https://res.cloudinary.com/${CLOUD_NAME}/image/list/${GALLERY_TAG}.json`)
        ...
        .then(data => {
            if (data && data.resources) {
                // Get permanently deleted IDs stored locally
                const deletedIds = JSON.parse(localStorage.getItem('jabaa_deleted_photos') || '[]');

                galleryPhotos = data.resources
                    .filter(img => !deletedIds.includes(img.public_id)) // Exclude deleted ones
                    .map((img, i) => ({
...
// PERMANENT DELETION LOGIC ADDED HERE
function confirmAndDeleteImage() {
    if (confirm("Suuraa kana haquu feetaa? (Are you sure you want to permanently delete this photo?")) {
        const photoToDelete = galleryPhotos[currentIndex];

        // Save deleted public ID to localStorage so it stays hidden on refresh
        if (photoToDelete && photoToDelete.publicId) {
            const deletedIds = JSON.parse(localStorage.getItem('jabaa_deleted_photos') || '[]');
            if (!deletedIds.includes(photoToDelete.publicId)) {
                deletedIds.push(photoToDelete.publicId);
                localStorage.setItem('jabaa_deleted_photos', JSON.stringify(deletedIds));
            }
        }

        // Remove from current runtime array and update UI
